...
const UpdateUserManagment = () => {
  ...
  const handleCheckboxChange = (_id) => {
    if (selectedMerchants && selectedMerchants.includes(_id)) {
      setSelectedMerchants((prevSelected) =>
        prevSelected.filter((merchantId) => merchantId !== _id),
      );
    } else {
      setSelectedMerchants((prevSelected) => [...prevSelected, _id]);
    }
  };

  const handleCheckboxChangeBrands = (_id) => {
    if (selectedBrands && selectedBrands.includes(_id)) {
      setSelectedBrands((prevSelected) => prevSelected.filter((brandId) => brandId !== _id));
    } else {
      setSelectedBrands((prevSelected) => [...prevSelected, _id]);
    }
  };
  ...
  return (
    <VStack spacing="6" align="stretch" p="4">
      <Flex justifyContent="space-between" alignItems="center">
        ...
        <Button onClick={updateUser} color="brand.text" bg="brand.secondary">
          {singleUser?.active ? 'Save' : 'Approve'}
        </Button>
      </Flex>
      <Box bg="brand.primary" p={5} borderRadius={10}>
        <form>
          <Flex flex={1} height={'50%'}>
            <Flex flexDirection={'column'} gap={2} flex={0.5} py={5} px={2} height={'300px'}>
              <Flex flex={1}>
                <Text flex={0.25} color="brand.lightText">
                  Name :
                </Text>
                <Text flex={0.5} color="brand.text" fontSize="large" textTransform={'capitalize'}>
                  {singleUser?.name}
                </Text>
              </Flex>
              <Flex flex={1}>
              ...
              <Flex flex={1}>
                ...
                <Text flex={0.5} color="brand.text" fontSize="large">
                  {singleUser?.email}
                </Text>
              </Flex>
              <Flex flex={1}>
            ...
            >
              <FormControl my={4}>
                <FormLabel htmlFor="Business_Unit">Select Business Unit</FormLabel>
                <Select
                  name="Business_Unit"
